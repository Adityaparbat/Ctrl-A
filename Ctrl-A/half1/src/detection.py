...
ACCESSIBILITY_OBJECTS = {
    ...
    'portrait scene': 'portrait scene',
    'indoor scene': 'indoor scene',
    'dark scene': 'dark scene',
    'low contrast scene': 'low contrast scene'
}
...
def _run_yolo_detection(image: Image.Image) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # Run inference with very low confidence threshold to catch more objects
        results = yolo_model.predict(img_array, verbose=False, conf=0.05, iou=0.2)
        ...
        if results and len(results) > 0:
            ...
            if result.boxes is not None:
                ...
                for i, (box, conf, cls_id) in enumerate(zip(boxes, confidences, class_ids)):
                    ...
                    # Much lower confidence threshold to catch more objects
                    if confidence_score > 0.05 and bbox_area > min_area:
        ...
        if not detections:
            ...
            for i, (box, conf, cls_id) in enumerate(zip(boxes, confidences, class_ids)):
                ...
                # Even lower threshold for fallback detection
                if confidence_score > 0.05 and bbox_area > min_area:
                    friendly_name = ACCESSIBILITY_OBJECTS.get(class_name, class_name)
                    detections.append({
                        'label': friendly_name,
        ...
        # If still no objects detected, try even lower confidence
        if not detections:
            logger.info("No objects detected with standard thresholds, trying very low confidence detection")
            for i, (box, conf, cls_id) in enumerate(zip(boxes, confidences, class_ids)):
                x1, y1, x2, y2 = box
                class_name = result.names.get(cls_id, f"class_{cls_id}")
                
                bbox_area = (x2 - x1) * (y2 - y1)
                min_area = total_area * 0.001  # Very small minimum area
                
                if conf > 0.1 and bbox_area > min_area:  # Lower confidence threshold for better accuracy
                    friendly_name = ACCESSIBILITY_OBJECTS.get(class_name, class_name)
                    detections.append({
                        'label': friendly_name,
        ...
        # If still no objects, use fallback
        if not detections:
...
def _fallback_object_detection(image: Image.Image) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if cv2_available:
            ...
            # 1) Fast person/face cues to avoid generic descriptions
            try:
                # Face detection (robust cue for 'person')
                face_cascade = cv2.CascadeClassifier(
                    os.path.join(cv2.data.haarcascades, 'haarcascade_frontalface_default.xml')
                )
                faces = face_cascade.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=5, minSize=(40, 40))
                for (fx, fy, fw, fh) in faces[:5]:
                    # Skip tiny faces that lead to "very small" narration
                    if (fw * fh) < (w * h) * 0.02:
                        continue
                    detections.append({
                        'label': 'person',
                ...
                logger.debug(f"Face detection error (ignored): {e}")

            try:
                # Pedestrian detector (HOG + SVM)
                hog = cv2.HOGDescriptor()
                hog.setSVMDetector(cv2.HOGDescriptor_getDefaultPeopleDetector())
                rects, weights = hog.detectMultiScale(gray, winStride=(8, 8), padding=(8, 8), scale=1.05)
                for (px, py, pw, ph), score in zip(rects[:5], (weights or [])):
                    # Avoid duplicating faces
                    area = pw * ph
                    if area < (w * h) * 0.03:
                        continue
                    detections.append({
                        'label': 'person',
                        'confidence': float(score[0]) if hasattr(score, '__len__') else 0.6,
                        'bbox': [float(px), float(py), float(px + pw), float(py + ph)]
                    })
            except Exception as e:
                logger.debug(f"HOG person detection error (ignored): {e}")
            ...
            # 2) Edges and contours for generic object shapes
            ...
            for contour in contours:
                ...
                if area > (w * h) * 0.005:  # Lower threshold to catch smaller objects
                    x, y, cw, ch = cv2.boundingRect(contour)
                    aspect_ratio = cw / max(1, ch)
                    
                    # More sophisticated classification based on size and shape
                    relative_area = area / (w * h)
                    
                    # Try to identify specific object types based on size, shape, and position
                    if relative_area > 0.15:
                        if aspect_ratio > 2:
                            label = "table"  # Large rectangular objects are often tables
                        elif aspect_ratio < 0.5:
                            label = "door"  # Tall objects could be doors
                        else:
                            label = "furniture"  # Large square objects
                    elif relative_area > 0.08:
                        if aspect_ratio > 1.5:
                            label = "chair"  # Medium rectangular objects
                        elif 0.8 <= aspect_ratio <= 1.2:
                            label = "basket"  # Medium square objects could be baskets
                        else:
                            label = "container"  # Medium objects
                    elif relative_area > 0.03:
                        if 0.8 <= aspect_ratio <= 1.2:
                            label = "bowl"  # Small round objects
                        elif aspect_ratio > 1.5:
                            label = "book"  # Small rectangular objects
                        else:
                            label = "small container"
                    else:
                        # Very small objects - could be fruits, small items
                        if 0.8 <= aspect_ratio <= 1.2:
                            label = "fruit"  # Small round objects are often fruits
                        elif aspect_ratio > 1.5:
                            label = "small item"  # Small rectangular objects
                        else:
                            label = "small object"
            ...
            if is_dark:
                ...
                detections.append({
                    'label': 'low contrast scene',
                    'confidence': 0.6,
                    'bbox': [0, 0, float(w), float(h)]
                })
            else:
                # Try to identify common objects based on image characteristics
                if w > h * 1.5:  # Wide image
                    detections.append({
                        'label': 'landscape scene',
                        'confidence': 0.5,
                        'bbox': [0, 0, float(w), float(h)]
                    })
                elif h > w * 1.5:  # Tall image
                    detections.append({
